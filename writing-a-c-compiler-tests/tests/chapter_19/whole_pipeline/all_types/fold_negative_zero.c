...
int target_not_neg_zero() {
    return !-0.0;  // 1
}

int target_neg_zero_and() {
    return -0.0 && 1;
}

/* Addition */
double target_add_negative_zero_to_self() {
    return -0. + -0.;  // negative (special case)
}

double target_add_positive_to_negative_zero() {
    // posiive (-0.0 + 0.0 ==> 0.0 -- 0.0, and x - x is always +0.0)
    return -0. + 0.;
}

double target_add_negative_to_positive_zero() {
    return 0. + -0.;  // positive; equivalent to previous case
}

double target_add_negative_nonzero_to_negative_zero() {
    // adding any number to negative zero results in the same number
    return -5.0 + -0.;
}

double target_add_positive_nonzero_to_negative_zero() {
    // adding any number to negative zero results in the same number
    return -0. + 10.;
}

/* Subtraction */
double target_subtract_neg_zero_from_self() {
    return -0. - -0.;  // positive zero; equivalent to 0. - 0.
}

double target_subtract_pos_zero_from_neg_zero() {
    return -0. - 0.;  // negative zero; equivalent to -0. + -0.
}

double target_subtract_neg_zero_from_pos_zero() {
    return 0. - -0.;  // positive zero; equivalent to 0. + 0.
}

double target_subtract_pos_nonzero_from_neg_zero() {
    return -0. - 10.;  // -10.
}

/* Multiplication */
double target_negative_zero_mult() {
    return -0.0 * 15.4e10;  // negative zero
}

double target_negative_zero_mult_negative() {
    return -100. * -0.;  // positive zero
}

double target_negative_zero_squared() {
    return -0. * -0.;  // positive zero
}

double target_neg_zero_mult_zero() {
    return 0. * -0.;  // negative zero
}

double target_mult_underflow() {
...
double target_div_neg_zero_by_pos_nonzero() {
    return -0. / 10.;  // negative zero
}

double target_div_pos_zero_by_neg_nonzero() {
    return 0. / -10.;  // negative zero
}

double target_div_neg_zero_by_neg_nonzero() {
    return -0. / -5.;  // positive zero
}

double target_div_negative_underflow() {
    // this result should underflow to negative zero
    return 0.5e-100 / -2e307;
}

double target_div_pos_non_zero_by_neg_zero() {
    return 10. / -0.0;  // negative infinity
}

double target_div_neg_nonzero_by_zero() {
    return -10. / 0.;  // negative infinity
}

double target_div_neg_nonzero_by_neg_zero() {
    return -100. / -0.;  // positive infinity
}

/* negation */
double target_negate_neg_zero() {
    return -(-0.0);  // positive zero
}

// we can calculate zero and negate it (i.e. we're not just treating the
// source-level expression -0.0 as a special case)
double target_negate_calculated_zero() {
    return -(50. - 50.);
}
